...
export function ReadingPaymentScreen({ cardCount, question, requestKey, onBack, onNext }: {
  ...
  return <section className="reading-page reading-step">
    <button className="text-action" onClick={onBack}>返回修改问题</button><h1>确认这次问事</h1>
    ...
    {error ? <p role="alert">{error}</p> : !resolution ? <p>正在确认可用权益…</p> : source ?
      ...
      <><p>当前可用权益不足，请先获取问事服务。</p><Link className="primary" href={`/shop?returnTo=${encodeURIComponent(`/readings/payment?cards=${cardCount}`)}`}>查看问事服务</Link></>}
