...
export function ReadingShuffle({ onBack, onNext }: { onBack: () => void; onNext: () => void }) {
  const deck=Array.from({length:17},(_,i)=>{const offset=i-8;return <i key={offset} style={{"--i":offset,"--rotation":`${offset*4.75}deg`} as React.CSSProperties}/>;});
...
export function ReadingDraw({ cards=[], onNext }: { cardCount?:number; cards?:CardReadingCard[]; onNext: () => void; onBack?: () => void }) {
  ...
  return <section className="reading-page reading-action-page single-draw-result"><ReadingHeader/><p className="eyebrow">THE CARD FOUND YOU</p><h1>{ready?"这张牌与你相遇":"正在从牌堆中抽取…"}</h1><div className="draw-deck-stage" aria-label="正在抽取卡牌"><div className="draw-deck">{Array.from({length:17},(_,i)=>{const offset=i-8;return <i key={offset} className={offset===picked?"is-picked":""} style={{"--i":offset,"--rotation":`${offset*4.75}deg`} as React.CSSProperties}>{offset===8&&<span className="card-mark">福</span>}</i>})}</div><div className="single-rising-card" style={{"--picked":picked,"--rotation":`${picked*4.75}deg`} as React.CSSProperties}><div className="draw-card-inner"><div className="draw-card-back"><span>福</span></div><div className="draw-card-front">{card?<img src={`/cards/satori-default-v1/${card.cardCode}.png`} alt={card.displayName}/>:<span>福</span>}</div></div></div></div><p className="draw-card-caption">{ready?"牌面已为你展开":"请稍候，牌正在回应你的问题"}</p>{ready&&<button className="primary draw-interpret" onClick={onNext}>开始解读 <span>→</span></button>}</section>;
}
