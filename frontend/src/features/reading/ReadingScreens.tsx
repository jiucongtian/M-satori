...
export function ReadingDraw({ cards=[], onNext }: { cardCount?:number; cards?:CardReadingCard[]; onNext: () => void; onBack?: () => void }) {
  const card=cards[0]; const [picked]=useState(()=>Math.floor(Math.random()*17)-8); const [animationDone,setAnimationDone]=useState(false); const angle=picked*4.75; const originX=Math.sin(angle*Math.PI/180)*192; const ready=animationDone&&!!card;
  useEffect(()=>{const timer=window.setTimeout(()=>setAnimationDone(true),4300);return()=>window.clearTimeout(timer)},[]);
  return <section className="reading-page reading-action-page single-draw-result"><ReadingHeader/><p className="eyebrow">THE CARD FOUND YOU</p><h1>{ready?"这张牌与你相遇":"正在从牌堆中抽取…"}</h1><div className="draw-deck-stage" aria-label="正在抽取卡牌"><div className="draw-deck">{Array.from({length:17},(_,i)=>{const offset=i-8;return <i key={offset} className={offset===picked?"is-picked":""} style={{"--i":offset,"--rotation":`${offset*4.75}deg`} as React.CSSProperties}>{offset===8&&<span className="card-mark">福</span>}</i>})}</div><div className="single-rising-card" style={{"--picked":picked,"--rotation":`${angle}deg`,"--origin-x":`${originX}px`,"--center-shift":`${-originX}px`} as React.CSSProperties}><div className="draw-card-inner"><div className="draw-card-back"><span>福</span></div><div className="draw-card-front">{card?<img src={`/cards/satori-default-v1/${CARD_ASSET_BY_CODE[card.cardCode] ?? card.cardCode.toLowerCase().replaceAll("_", "") + ".jpg"}`} alt={card.displayName}/>:<span>福</span>}</div></div></div></div><p className="draw-card-caption" aria-hidden="true"></p>{ready&&<button className="primary draw-interpret" onClick={onNext}>开始解读 <span>→</span></button>}</section>;
}
