...
export function ReadingShuffle({ onBack, onNext }: { onBack: () => void; onNext: () => void }) {
  return <section className="reading-page immersive-reading reading-action-page"><ReadingHeader onBack={onBack}/><p className="eyebrow">YOUR QUESTION · 准备抽卡</p><h1>把此刻关心的事<br/>轻轻放在心里</h1><p>不需要说出来。深呼吸一次，<br/>准备好后，抽一张牌。</p><div className="draw-deck-stage poc-draw-idle" aria-hidden="true"><div className="draw-deck">{Array.from({length:17},(_,i)=>{const offset=i-8;return <i key={offset} style={{"--i":offset,"--rotation":`${offset*4.75}deg`} as React.CSSProperties}><img src="/cards/wufuhui-card-back.jpg" alt="" /></i>})}</div></div><button className="primary" onClick={onNext}>抽一张牌 <span>→</span></button><small>点击牌面也可以抽牌</small></section>;
}
