...
export default function ReadingHomeScreen(){
  ...
  useEffect(()=>{
    const reset=()=>document.querySelector<HTMLElement>(".reading-home-scroll")?.scrollTo({top:0,left:0,behavior:"instant"});
    const resetWhenVisible=()=>{if(document.visibilityState==="visible")reset()};
    reset();
    window.addEventListener("pageshow",reset);
    document.addEventListener("visibilitychange",resetWhenVisible);
    return()=>{window.removeEventListener("pageshow",reset);document.removeEventListener("visibilitychange",resetWhenVisible)};
  },[]);
