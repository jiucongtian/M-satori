...
export default function ReadingNewScreen(){
  ...
  useEffect(()=>{if(owner==="unknown"||restored)return;const timer=window.setTimeout(()=>{const draft=readFlowDraft<ReadingDraft>("reading",owner,DRAFT_VERSION);if(draft){setQuestion(draft.question);setCardCount(1);setPositions(["自己"])}setRestored(true)},0);return()=>window.clearTimeout(timer)},[owner,restored]);
  ...
  return <ProtectedRoute><RouteFrame title="开始问事" label="问题、牌数与牌位"><section className="reading-page reading-compose"><PageDebugLabel>R1.1 · READ-02</PageDebugLabel><ReadingHeader backHref={ROUTES.readings}/><div className="reading-compose-scroll">
    ...
    <section className="compose-section position-section"><header><small>本次抽卡</small><h2>一张卡 · 核心指引</h2><p>把此刻最关心的事交给一张牌，听听它带来的提醒。</p></header></section>
  ...
  </div><button className="primary compose-submit" type="button" disabled={!ready} onClick={()=>{window.sessionStorage.removeItem(`fresh:reading-request:${owner}`);router.push(`/readings/payment?cards=${cardCount}`)}}>确认问题，抽一张卡 <span>→</span></button><p className="next-hint">下一步确认本次使用的问事权益；返回时内容仍会保留</p></section></RouteFrame></ProtectedRoute>;
}
