...
const categories=["事业","情感","财富","健康","选择","个人状态","其他"];
...
export default function ReadingNewScreen(){
  ...
  const[positions,setPositions]=useState(()=>positionTemplate("事业",1,"recommended"));
  const[restored,setRestored]=useState(false);

  useEffect(()=>{if(owner==="unknown"||restored)return;const timer=window.setTimeout(()=>{const draft=readFlowDraft<ReadingDraft>("reading",owner,DRAFT_VERSION);if(draft){setQuestion(draft.question);setCategory(draft.category);setCardCount(draft.cardCount);setMode(draft.mode);setPositions(draft.positions)}setRestored(true)},0);return()=>window.clearTimeout(timer)},[owner,restored]);
  ...
  function chooseCategory(value:string){setCategory(value);setMode("recommended");setPositions(positionTemplate(value,cardCount,"recommended"))}
  ...
  return <ProtectedRoute><RouteFrame title="开始问事" label="问题、牌数与牌位"><section className="reading-page reading-compose"><PageDebugLabel>R1.1 · READ-02</PageDebugLabel><ReadingHeader backHref={ROUTES.readings}/><div className="reading-compose-scroll"><p className="eyebrow">YOUR QUESTION</p><h1>此刻，你最想问什么？</h1><p className="reading-lead">把问题说清楚，再决定希望每张牌帮你看见什么。</p><div className="question-box"><textarea value={question} maxLength={120} placeholder="例如：面对目前的工作变化，我可以如何找到更适合自己的方向？" onChange={event=>setQuestion(event.target.value)} aria-label="输入想问的问题"/><div><small>{question.length} / 120</small><button type="button" onClick={()=>setQuestion("")}>清空</button></div></div><div className="question-types"><small>这个问题更接近哪个方向？</small><div>{categories.map(item=><button type="button" key={item} className={category===item?"active":""} onClick={()=>chooseCategory(item)}>{item}</button>)}</div></div><section className="compose-section"><header><small>STEP 02</small><h2>这次想用几张牌来看？</h2><p>数量不是越多越好，选择最贴近问题的观察方式。</p></header><div className="compose-counts">{[{count:1,label:"一张",note:"聚焦一份核心提醒"},{count:2,label:"两张",note:"看双方、两面或两个选择"},{count:3,label:"多张",note:"展开时间、人物或多个方案"}].map(item=>{const active=item.count===3?cardCount>=3:cardCount===item.count;return <button type="button" className={active?"active":""} key={item.count} onClick={()=>chooseCount(item.count)}><span>{item.label}</span><small>{item.note}</small><b>{active?"✓":""}</b></button>})}</div>{cardCount>=3&&<div className="compose-multi-count"><span>具体抽几张？</span>{[3,4,5].map(value=><button type="button" key={value} className={cardCount===value?"active":""} onClick={()=>chooseCount(value)}>{value} 张</button>)}</div>}</section>{cardCount>1&&<section className="compose-section position-section"><header><small>STEP 03</small><h2>每张牌分别代表什么？</h2><p>系统结合你的问题给出建议，你可以直接确认，也可以修改。</p></header><div className="position-modes">{([["recommended","为我推荐"],["timeline","按时间展开"],["options","比较多个选择"],["custom","自己定义"]] as [PositionMode,string][]).map(([value,label])=><button type="button" key={value} className={mode===value?"active":""} onClick={()=>chooseMode(value)}>{label}</button>)}</div><div className="position-editor">{positions.map((value,index)=><label key={index}><span>第 {index+1} 张</span><input value={value} maxLength={20} aria-label={`第 ${index+1} 张牌的含义`} onChange={event=>editPosition(index,event.target.value)}/></label>)}</div></section>}<section className="compose-summary"><small>本次问事</small><p><span>方向</span><strong>{category}</strong></p><p><span>数量</span><strong>{cardCount} 张牌</strong></p>{cardCount>1&&<p><span>牌位</span><strong>{positions.join(" · ")}</strong></p>}</section></div><button className="primary compose-submit" type="button" disabled={!ready} onClick={()=>router.push(`/readings/payment?cards=${cardCount}`)}>确认问题{cardCount>1?"与牌位":""} <span>→</span></button><p className="next-hint">下一步确认本次使用的问事权益；返回时内容仍会保留</p></section></RouteFrame></ProtectedRoute>;
}
