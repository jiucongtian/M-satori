...
export default function ReadingNewScreen(){
  ...
  </div><button className="primary compose-submit" type="button" disabled={!ready} onClick={()=>router.push(`/readings/payment?cards=${cardCount}`)}>确认问题与牌位 <span>→</span></button><p className="next-hint">下一步确认本次使用的问事权益；返回时内容仍会保留</p></section></RouteFrame></ProtectedRoute>;
}
