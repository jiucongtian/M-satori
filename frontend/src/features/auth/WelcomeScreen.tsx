...
import { routeAfterAuthentication } from "./navigation";

export default function WelcomeScreen() {
  const [started, setStarted] = useState(false); const { status, me } = useSession(); const router = useRouter();
  useEffect(() => { if (status === "authenticated") router.replace(routeAfterAuthentication(me?.nextAction)); }, [me?.nextAction, router, status]);
