...
export default function ShareFirstLookScreen({step}:{step:ShareStep}){
  ...
  async function save(){const node=document.querySelector<HTMLElement>(".poster-large .fresh-poster");if(!node)return;setSaving(true);setSaveError("");try{await document.fonts.ready;let dataUrl="";for(const pixelRatio of [2,1.5]){try{dataUrl=await toJpeg(node,{cacheBust:true,width:360,height:640,pixelRatio,quality:.92,backgroundColor:"#f5f1e7",style:{transform:"none"}});if(dataUrl.length<900_000)break}catch{dataUrl=""}}if(!dataUrl)throw new Error("POSTER_EXPORT_FAILED");const blob=await fetch(dataUrl).then(response=>response.blob());const file=new File([blob],`分享初见-${item.id}.jpg`,{type:"image/jpeg"});if(navigator.share&&navigator.canShare?.({files:[file]})){await navigator.share({files:[file],title:"分享初见"})}else if(/MicroMessenger|iPhone|iPad|Android/i.test(navigator.userAgent)){const uploaded=await api.uploadSharePoster(dataUrl);setSavePreview(`/api/v1/share-posters/${encodeURIComponent(uploaded.posterId)}.jpg`)}else{const link=document.createElement("a");link.download=file.name;link.href=dataUrl;document.body.appendChild(link);link.click();link.remove()}setSaved(true);track("share_poster_saved",{result:"success",object_type:kind==="daily"?"daily_insight":"reading",object_id:(kind==="daily"?date:readingId)??undefined,properties:{poster_style:item.id,format:"jpeg"}})}catch(error){if(error instanceof DOMException&&error.name==="AbortError")return;setSaved(false);setSaveError("海报暂时没有保存成功，请稍后重试或更换浏览器打开。");track("share_poster_saved",{result:"failed",reason_code:"POSTER_EXPORT_OR_UPLOAD_FAILED",properties:{poster_style:item.id}})}finally{setSaving(false)}}
