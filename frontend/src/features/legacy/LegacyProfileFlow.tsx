...
export function hasCompleteFirstLookCards(value: ProfileRevision | null): boolean {
...
export function LegacyProfileFlow({ onExit, onLogout, onNavigateRoute, initialStep = 0, dailyDate }: { onExit: () => void; onLogout: () => void | Promise<void>; onNavigateRoute?: (path: string, replace?: boolean) => void; initialStep?: number; dailyDate?: string }) {
  ...
  useEffect(() => {
    if (!dailyDate || initialStep !== 14) return;
    let active = true;
    api.dailyInsight(dailyDate).then((value) => { if (active) setDailyInsight(value); }).catch((error) => { if (active) setApiError(apiMessage(error)); });
    return () => { active = false; };
  }, [dailyDate, initialStep]);
  ...
  return (
    <div className={`profile-flow${id === "HOME-01" ? " home-flow" : ""}${headerlessSteps.has(id) ? " headerless-flow" : ""}`}>
      ...
      {id === "READ-09" && <ReadingPayment onBack={back} onNext={next} />}
...
export function ProfileIntro({ onNext }: { onNext: () => void }) {
...
export function UnifiedProfileForm({ data, onChange, onNext, variant = "self", mode = "create", busy = false }: { data: ProfileData; onChange: (data: ProfileData) => void; onNext: () => void; variant?: "self" | "other"; mode?: "create" | "edit"; busy?: boolean }) {
...
export function RelationshipFirstLook({ name, revision, report, loading, onRetry, onEdit, onSkip, onNext }: { name: string; revision: ProfileRevision | null; report: ProfileFirstLook | null; loading: boolean; onRetry: () => void; onEdit: () => void; onSkip: () => void; onNext: () => void }) {
...
export function SeedGift({ name, amount, claimed = false, busy, onClaim, onNext }: { name: string; amount: number; claimed?: boolean; busy: boolean; onClaim: () => void; onNext: () => void }) {
...
export function DailyStart({ name, energyLevel, balance, costLabel = "1 颗智慧种子", onBack, onNext }: { name: string; energyLevel?: EnergyLevel; balance: number | null; costLabel?: string; onBack: () => void; onNext: () => void }) {
  return <section className="daily-page daily-start"><DailyHeader onBack={onBack} balance={balance} /><div className="daily-seed-scene" aria-hidden="true"><i /><span>{energyLevel ?? "—"}</span><b>今日能量</b></div><p className="eyebrow">DAILY GUIDANCE</p><h1>{name}，今天的能量<br />正在邀请你慢下来</h1><p className="daily-lead">结合你的生命智慧档案与今日节律，为你整理一份只属于今天的行动指引。</p><div className="will-get"><small>你将获得</small><span><b>01</b>今日整体能量</span><span><b>02</b>事业与外部节奏</span><span><b>03</b>情绪与个人状态</span><span><b>04</b>一个可完成的小行动</span></div><div className="cost-preview"><span><i>●</i><small>本次使用服务权益</small></span><strong>{costLabel}</strong></div><button className="primary" type="button" onClick={onNext}>继续开启 <span>→</span></button></section>;
}

export function SeedPayment({ balance, busy, unified = false, onBack, onNext, onSupport }: { balance: number | null; busy: boolean; unified?: boolean; onBack: () => void; onNext: () => void; onSupport: () => void }) {
  const [helpOpen, setHelpOpen] = useState(false);
  const syncing = balance === null;
  const insufficient = !unified && !syncing && balance < 1;
  return <section className="daily-page seed-payment"><DailyHeader onBack={onBack} balance={balance} /><p className="eyebrow">CONFIRM SERVICE BENEFIT</p><h1>{unified ? "确认本次服务权益" : "使用一颗智慧种子"}</h1><p className="daily-lead">{unified ? "系统按固定顺序使用当前会员权益、已购权益包或智慧种子，你不需要手动选择。" : "智慧种子是平台附赠的 AI 体验额度，让每一次使用都成为一次更清晰的看见。"}</p><div className="pay-seed" aria-hidden="true"><i /><span>{unified ? "权" : "●"}</span><small>{unified ? "系统自动选择" : "智慧种子"}</small></div><div className="payment-card"><div><span>今日能量指引</span><strong>{unified ? "使用 1 次可用权益" : "使用 1 颗"}</strong></div><div><span>固定顺序</span><strong>{unified ? "会员 → 权益包 → 智慧种子" : `当前 ${balance ?? "—"} 颗`}</strong></div><div className="after"><span>结算规则</span><strong>{unified ? "成功核销，失败自动释放" : `使用后 ${balance === null ? "—" : Math.max(0, balance - 1)} 颗`}</strong></div></div><p className="payment-note"><span className="lock" />确认后由服务端预留；未形成有效内容会自动恢复。</p><button className={`primary ${insufficient ? "credit-help-trigger" : ""}`} type="button" disabled={busy || syncing} onClick={insufficient ? () => setHelpOpen(true) : onNext}>{busy ? "正在提交…" : syncing ? "正在同步可用权益…" : insufficient ? "AI 体验额度不足，获取帮助" : unified ? "确认使用系统选定权益" : "确认使用 1 颗智慧种子"} <span>{insufficient ? "→" : "●"}</span></button><button className="text-action" type="button" onClick={onBack}>再想一想</button>{helpOpen && <div className="credit-help-backdrop" role="presentation" onClick={() => setHelpOpen(false)}><section className="credit-help-sheet" role="dialog" aria-modal="true" aria-labelledby="credit-help-title" onClick={(event) => event.stopPropagation()}><span className="sheet-icon" aria-hidden="true">芽</span><p className="eyebrow">A LITTLE SUPPORT</p><h2 id="credit-help-title">本次体验额度暂时不足</h2><p>你可以联系官方客服，了解体验额度的获取方式。当前内容不会丢失，获得额度后可以回来继续。</p><button className="primary" type="button" onClick={onSupport}>联系官方客服 <span>→</span></button><button className="text-action" type="button" onClick={() => setHelpOpen(false)}>稍后再说</button></section></div>}</section>;
}

export function DailyGenerating({ name, balance, onBack }: { name: string; balance: number | null; onBack: () => void }) {
...
export function DailyReport({ name, insight, energyLevel, heavenCard, balance, onBack, onNext }: { name: string; insight: DailyInsight | null; energyLevel?: EnergyLevel; heavenCard?: string; balance: number | null; onBack: () => void; onNext: () => void }) {
  const content = insight?.content;
  return <section className="daily-page daily-report"><DailyHeader onBack={onBack} balance={balance} /><div className="report-scroll"><p className="eyebrow">TODAY · {insight?.localDate || "今日"}{heavenCard ? ` · ${heavenCard}` : ""}</p><h1>{name}的今日能量指引</h1><div className="energy-header"><div><span>{energyLevel ?? "—"}</span><small>今日能量</small></div><p><small>今日关键词</small><strong>{content?.theme || "回到自己的节奏"}</strong></p></div><article className="report-opening"><b>今日总览</b><h2>{content?.theme || "把重要的事，放在心静之后"}</h2><p>{content?.insight || insight?.fallback?.message || "今天的指引已经生成。"}</p></article><div className="report-columns"><article><small>今日行动</small><h3>从一件小事开始</h3><p>{content?.action || "为自己留出一点安静的时间。"}</p></article><article><small>今日反思</small><h3>问问自己</h3><p>{content?.reflectionQuestion || "此刻对我真正重要的是什么？"}</p></article></div><AiContentNotice /><button className="primary" type="button" onClick={onNext}>收下今天的行动 <span>→</span></button></div></section>;
}
...
export function ReadingPayment({ cardCount=2, question, category="事业", positions, onBack, onNext }: { cardCount?:number; question?:string; category?:string; positions?:string[]; onBack: () => void; onNext: () => void }) {
  return <ReadingStep onBack={onBack} eyebrow="CONFIRM YOUR READING" title="确认这次问事" lead="无论抽取几张牌，一次完整问事统一使用 1 次问事权益。" action="使用 1 次问事权益，开始抽牌" onNext={onNext}><div className="reading-order"><small>本次内容</small><h2>{category} · {cardCount} 张卡牌</h2><p>{question||"面对当前的问题，我可以如何看见更适合自己的方向？"}</p>{positions?.length&&<div><span>牌位含义</span><strong>{positions.join(" · ")}</strong></div>}<div><span>卡牌体系</span><strong>默认生命智慧卡牌</strong></div><div><span>抽牌方式</span><strong>系统公平随机抽取</strong></div><div className="cost"><span>本次使用</span><strong>问事权益 1 次</strong></div></div><div className="balance-change"><span>服务包可用 <b>8 次</b></span><i>→</i><span>使用后 <b>7 次</b></span></div><p className="refund-note">只有形成有效问事报告才会核销；生成失败不会消耗权益，也不会重复提交。</p></ReadingStep>;
}
...
export function ReadingReveal({ cardCount=2, onBack, onNext }: { cardCount?:number; onBack: () => void; onNext: () => void }) {
  const cards=[["18-xinsi.jpg","辛巳"],["01-jiazi.jpg","甲子"],["41-jiachen.jpg","甲辰"],["07-gengwu.jpg","庚午"],["25-wuzi.jpg","戊子"]].slice(0,cardCount);
  return <ReadingStep onBack={onBack} eyebrow="YOUR CARDS" title="牌已经回应了你的问题" lead={`确认后，这 ${cardCount} 张牌将冻结并用于生成报告。`} action={`确认 ${cardCount} 张卡牌并生成报告`} onNext={onNext}><div className={`card-layout report-card-gallery reveal-card-gallery count-${cardCount}`}>{cards.map(([file,name])=><figure key={file}><img src={`/cards/satori-default-v1/${file}`} alt={`${name}生命智慧卡牌`}/></figure>)}</div><div className="frozen-note"><span>封</span><p><strong>本次输入即将冻结</strong>问题、抽牌方式与全部卡牌会被共同保存。</p></div></ReadingStep>;
}

export function ReadingGenerate({ cardCount=2, onBack, onSuccess, onFailure, onLeave, onNetworkError }: { cardCount?:number; onBack: () => void; onSuccess: () => void; onFailure: () => void; onLeave: () => void; onNetworkError: () => void }) {
  const [complete,setComplete]=useState(false);
  useEffect(()=>{const timer=window.setTimeout(()=>setComplete(true),1200);return()=>window.clearTimeout(timer)},[]);
  useEffect(()=>{if(!complete)return;const timer=window.setTimeout(onSuccess,450);return()=>window.clearTimeout(timer)},[complete,onSuccess]);
  const files=["18-xinsi.jpg","01-jiazi.jpg","41-jiachen.jpg","07-gengwu.jpg","25-wuzi.jpg"].slice(0,cardCount);
  return <section className="reading-page reading-generating reading-action-page"><ReadingHeader onBack={onBack}/><div className={`card-layout generation-card-stage count-${cardCount}`}>{files.map((file,i)=><figure key={file}><img src={`/cards/satori-default-v1/${file}`} alt={`第 ${i+1} 张生命智慧卡牌`}/></figure>)}</div><p className="eyebrow">READING YOUR CARDS</p><h1>{complete?"报告已经生成完成":`正在读懂这 ${cardCount} 张牌`}</h1><div className="generation-list"><span className="done">✓ 固定问题与 {cardCount} 张卡牌</span><span className="done">✓ 同步后端抽牌结果</span><span className={complete?"done":"active"}>{complete?"✓":"·"} 整理五段式问事报告</span><span className={complete?"done":""}>{complete?"✓":"·"} 完成报告与种子核销</span></div><p>{complete?"即将自动打开报告详情":"可以离开页面，完成后会在问事记录中保存"}</p><button className="outline-button" onClick={onLeave}>先离开，稍后查看</button><button className="prototype-failure" onClick={onFailure}>查看生成失败状态</button><button className="prototype-failure" onClick={onNetworkError}>查看网络中断状态</button></section>;
}
...
export function ReadingReport({ cardCount=2, onBack, onNext }: { cardCount?:number; onBack: () => void; onNext: () => void }) {
  const cards=[["18-xinsi.jpg","辛巳"],["01-jiazi.jpg","甲子"],["41-jiachen.jpg","甲辰"],["07-gengwu.jpg","庚午"],["25-wuzi.jpg","戊子"]].slice(0,cardCount);
  return <section className="reading-page reading-report"><ReadingHeader onBack={onBack}/><div className="reading-report-scroll"><p className="eyebrow">YOUR READING · {cardCount} CARDS</p><h1>变化不是在催你离开<br/>而是在邀请你重新选择</h1><div className={`card-layout report-card-gallery count-${cardCount}`}>{cards.map(([file,name])=><figure key={file}><img src={`/cards/satori-default-v1/${file}`} alt={`${name}生命智慧卡牌`}/></figure>)}</div>{[["01 · 先说结论","你面对的并不是一个必须立刻做出决定的时刻。真正重要的，是辨认哪些变化值得回应，哪些只是外界的噪声。"],["02 · 此刻的你","你对工作中的价值与秩序非常敏锐，所以变化越多，越容易担心自己是否失去了原来的位置。"],["03 · 牌想对你说",`这 ${cardCount} 张牌共同呈现出从看见变化、理解内心到重新选择的过程。你不必守住过去的答案。`],["04 · 可以试试","今天先写下：我想保留什么、愿意放下什么、下一步只验证什么。不要一次解决所有问题。"],["05 · 写给你的话","方向不是在焦虑中想出来的，而是在一次次诚实选择里逐渐清晰。"]].map(([h,p])=><article key={h}><h2>{h}</h2><p>{p}</p></article>)}<button className="primary" onClick={onNext}>完成阅读，返回问事首页 <span>→</span></button></div></section>;
}
...
function MainNav({ active, navigate }: { active: string; navigate: (step: number) => void }) {
  const tabs = [["今日", 10, "◉"], ["问事", profileSteps.indexOf("PREVIEW-READ"), "◇"], ["关系", profileSteps.indexOf("PREVIEW-RELATIONSHIP"), "∞"], ["成长", profileSteps.indexOf("PREVIEW-GROWTH"), "❧"], ["我的", 21, "○"]] as const;
...
function HomeMainNav({ active, navigate }: { active: string; navigate: (target: HomeNavTarget) => void }) {
  const destinations: Record<AppTab, HomeNavTarget> = { 今日: "today", 问事: "reading", 关系: "relationship", 成长: "growth", 我的: "my" };
  return <AppBottomNav active={active as AppTab} onNavigate={(tab) => navigate(destinations[tab])}/>;
}
...
export function ComingSoonPage({ kind, navigate }: { kind: ComingSoonKind; navigate: (target: HomeNavTarget) => void }) {
  ...
  return <section className="coming-soon-page"><div className="coming-soon-scroll">
    ...
    </div><HomeMainNav active={kind} navigate={navigate}/>
  </section>;
}
...
export function MyHome({ name, balance, serviceSummary, open, navigate }: { name: string; balance: number | null; serviceSummary?: MyServiceSummary; open: (action: string) => void; navigate: (target: HomeNavTarget) => void }) {
  const items = [["账号与退出", "管理当前账号登录状态", "settings", "隐"], ["联系我们", "官方社交媒体与客服渠道", "support", "联"]] as const;
  const membership = serviceSummary?.membership;
  const daily = serviceSummary?.daily;
  const reading = serviceSummary?.reading;
  const expiryLabel = (value: string | null) => value ? `${new Intl.DateTimeFormat("zh-CN", { timeZone: "Asia/Shanghai" }).format(new Date(value))} 到期` : "暂无有效批次";
  return <section className="my-page my-home"><div className="my-home-scroll"><header><Brand compact /></header><button className="my-identity" type="button" onClick={() => open("profile")} aria-label="查看我的生命智慧档案"><div className="avatar-seed"><i /><span>{(name || "我").slice(0,1)}</span></div><div><p>你好</p><h1>{name || "我"}</h1><small>生命智慧档案已建立</small></div><b>›</b></button><button className="profile-banner" type="button" onClick={() => open("archive")} aria-label="进入生命智慧档案库"><div><small>MY LIFE WISDOM ARCHIVE</small><h2>生命智慧档案库</h2><p>查看和管理自己与重要之人的档案</p><strong className="profile-banner-action">进入档案库 <span>→</span></strong></div><div className="four-dots"><i /><i /><i /><i /></div><b>›</b></button><section className="my-r11-service-center"><header><div><small>MY SERVICES</small><h2>我的服务</h2></div><button type="button" onClick={() => open("orders")}>我的订单 ›</button></header><button className={`my-current-membership ${serviceSummary?"":"loading"}`} type="button" onClick={() => open("membership")} aria-busy={!serviceSummary}><div className="membership-mark"><span>{membership?.planName.slice(0,1) ?? "会"}</span><i/></div><div><small>{!serviceSummary?"正在读取会员服务":membership?"当前会员服务":"当前尚未开通会员"}</small><strong>{!serviceSummary?"会员服务":membership?.planName ?? "选择会员计划"}</strong><p>{!serviceSummary?"稍候为你呈现":membership?`本周期还剩 ${membership.remainingDays} 天`:"可开通微光、清和或自在计划"}</p></div><em>{!serviceSummary?"读取中":membership?"正在使用":"去开通"}</em><b>›</b></button><section className={`my-quota-summary ${serviceSummary?"":"loading"}`} aria-label="我的服务权益" aria-busy={!serviceSummary}><header><strong>我的权益</strong><button type="button" onClick={() => open("benefits")}>权益明细 ›</button></header><div><button type="button" onClick={() => open("benefits")}><span>日</span><p><small>今日能量</small><strong>{daily?.available??"—"}<em>{daily?` / ${daily.total}次`:""}</em></strong><b>{daily?expiryLabel(daily.expiresAt):"正在读取"}</b></p></button><button type="button" onClick={() => open("benefits")}><span>问</span><p><small>抽卡问事</small><strong>{reading?.available??"—"}<em>{reading?` / ${reading.total}份`:""}</em></strong><b>{reading?expiryLabel(reading.expiresAt):"正在读取"}</b></p></button></div></section><button className="my-seed-credit-secondary" type="button" onClick={() => open("seeds")}><span>芽</span><div><small>平台赠送 · AI体验额度</small><strong>智慧种子 {balance ?? "—"} 颗</strong><p>可体验指定AI功能，也可解锁指定活动价</p></div><b>明细 ›</b></button><p className="my-seed-boundary">不可购买、交易、提现或兑换人民币；活动价不等于现金抵扣。</p><div className="my-commerce-shortcuts r11"><button type="button" onClick={() => open("membership")}><i>会</i><span><strong>会员计划</strong><small>比较微光、清和、自在</small></span><b>›</b></button><button type="button" onClick={() => open("services")}><i>选</i><span><strong>服务商城</strong><small>购买明确服务与次数包</small></span><b>›</b></button></div></section><div className="my-menu">{items.map(([title,note,action,icon]) => <button type="button" key={title} onClick={() => open(action)}><i>{icon}</i><span><strong>{title}</strong><small>{note}</small></span><b>›</b></button>)}</div></div><HomeMainNav active="我的" navigate={navigate} /></section>;
}
...
export function MySeeds({ account, transactions, onBack }: { account: WisdomSeedAccount | null; transactions: WisdomSeedTransaction[]; onBack: () => void }) {
  ...
  return <section className="my-page my-detail my-seeds-page"><MyHeader title="我的智慧种子" onBack={onBack} /><div className="seed-wallet"><small>可用 AI 体验额度</small><strong>{account?.available ?? "—"}</strong><span>●</span><p>每一颗种子，都可以开启一次新的看见</p></div><div className="asset-tabs" role="tablist" aria-label="智慧种子记录分类">{["最近记录","获得","使用"].map(x=><button type="button" role="tab" aria-selected={tab===x} key={x} className={tab===x?"active":""} onClick={()=>setTab(x)}>{x}</button>)}</div><div className="asset-list">{shown.map(x=><article key={`${x.title}-${x.time}`}><i>{x.icon}</i><span><strong>{x.title}</strong><small>{x.time}</small></span><b className={x.value.startsWith("+")?"plus":""}>{x.value}</b></article>)}</div>{shown.length===0&&<div className="prototype-empty">这一分类暂时没有记录</div>}<p className="asset-rule">智慧种子来自平台免费赠送、会员附赠或学院配置，仅用于 AI 体验，不可购买、充值、提现、转赠或交易。额度及使用记录来自当前后端账户。</p></section>;
}

export function MyReports({ home, insight, onBack,onDaily }: { home: HomeOverview | null; insight: DailyInsight | null; onBack: () => void;onDaily:()=>void }) {
  const ready = home?.dailyInsight.state === "READY";
