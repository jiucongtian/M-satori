...
export function MyHome({ name, balance, open, navigate }: { name: string; balance: number | null; open: (step: number) => void; navigate: (target: HomeNavTarget) => void }) {
  ...
  return <section className="my-page my-home"><header><Brand compact /></header><button className="my-identity" type="button" onClick={() => open(22)} aria-label="查看我的生命智慧档案"><div className="avatar-seed"><i /><span>{(name || "我").slice(0,1)}</span></div><div><p>你好</p><h1>{name || "我"}</h1><small>生命智慧档案已建立</small></div><b>›</b></button><button className="profile-banner" type="button" onClick={() => open(111)} aria-label="进入生命智慧档案库"><div><small>MY LIFE WISDOM ARCHIVE</small><h2>生命智慧档案库</h2><p>查看和管理自己与重要之人的档案</p><strong className="profile-banner-action">进入档案库 <span>→</span></strong></div><div className="four-dots"><i /><i /><i /><i /></div><b>›</b></button><div className="my-assets"><button type="button" onClick={() => open(23)}><span>●</span><div><small>智慧种子 · AI 体验额度</small><strong>{balance ?? "—"}<em> 颗可用</em></strong></div><b>查看明细 ›</b></button></div><div className="my-menu">{items.map(([title,note,step,icon]) => <button type="button" key={title} onClick={() => open(step)}><i>{icon}</i><span><strong>{title}</strong><small>{note}</small></span><b>›</b></button>)}</div><HomeMainNav active="我的" navigate={navigate} /></section>;
}
