...
import type { BirthInput, CardReadingCard, DailyInsight, HomeOverview, LifeProfile, ProfileFirstLook, ProfileRevision, WisdomSeedAccount, WisdomSeedTransaction } from "@/src/api/client";
...
export function DailyReport({ name, insight, energyLevel, heavenCard, balance, onBack, onNext, onShare }: { name: string; insight: DailyInsight | null; energyLevel?: EnergyLevel; heavenCard?: string; balance: number | null; onBack: () => void; onNext: () => void; onShare?: () => void }) {
  const content = insight?.content;
  return <section className="daily-page daily-report"><DailyHeader onBack={onBack} balance={balance} /><div className="report-scroll"><p className="eyebrow">TODAY · {insight?.localDate || "今日"}{heavenCard ? ` · ${heavenCard}` : ""}</p><h1>{name}的今日能量指引</h1><div className="energy-header"><div><span>{energyLevel ?? "—"}</span><small>今日能量</small></div><p><small>今日关键词</small><strong>{content?.theme || "回到自己的节奏"}</strong></p></div><article className="report-opening"><b>今日总览</b><h2>{content?.theme || "把重要的事，放在心静之后"}</h2><p>{content?.insight || insight?.fallback?.message || "今天的指引已经生成。"}</p></article><div className="report-columns"><article><small>今日行动</small><h3>从一件小事开始</h3><p>{content?.action || "为自己留出一点安静的时间。"}</p></article><article><small>今日反思</small><h3>问问自己</h3><p>{content?.reflectionQuestion || "此刻对我真正重要的是什么？"}</p></article></div><AiContentNotice />{onShare&&<button className="outline-button" type="button" onClick={onShare}>分享初见 <span>↗</span></button>}<button className="primary" type="button" onClick={onNext}>收下今天的行动 <span>→</span></button></div></section>;
}
...
export function ReadingGenerate({ cardCount=2, cards=[], onBack, onSuccess, onFailure, onLeave, onNetworkError }: { cardCount?:number; cards?:CardReadingCard[]; onBack: () => void; onSuccess: () => void; onFailure: () => void; onLeave: () => void; onNetworkError: () => void }) {
...
export function ReadingReport({ cardCount=2, cards=[], onBack, onNext, onShare }: { cardCount?:number; cards?:CardReadingCard[]; onBack: () => void; onNext: () => void; onShare?: () => void }) {
  return <section className="reading-page reading-report"><ReadingHeader onBack={onBack}/><div className="reading-report-scroll"><p className="eyebrow">YOUR READING · {cardCount} CARDS</p><h1>变化不是在催你离开<br/>而是在邀请你重新选择</h1><div className={`card-layout report-card-gallery count-${cardCount}`}>{cards.map(card=><figure key={card.cardCode}><img src={`/cards/satori-default-v1/${card.cardCode}.jpg`} alt={`${card.displayName}生命智慧卡牌`}/></figure>)}</div>{[["01 · 先说结论","你面对的并不是一个必须立刻做出决定的时刻。真正重要的，是辨认哪些变化值得回应，哪些只是外界的噪声。"],["02 · 此刻的你","你对工作中的价值与秩序非常敏锐，所以变化越多，越容易担心自己是否失去了原来的位置。"],["03 · 牌想对你说",`这 ${cardCount} 张牌共同呈现出从看见变化、理解内心到重新选择的过程。你不必守住过去的答案。`],["04 · 可以试试","今天先写下：我想保留什么、愿意放下什么、下一步只验证什么。不要一次解决所有问题。"],["05 · 写给你的话","方向不是在焦虑中想出来的，而是在一次次诚实选择里逐渐清晰。"]].map(([h,p])=><article key={h}><h2>{h}</h2><p>{p}</p></article>)}{onShare&&<button className="outline-button" type="button" onClick={onShare}>分享初见 <span>↗</span></button>}<button className="primary" onClick={onNext}>完成阅读，返回问事首页 <span>→</span></button></div></section>;
}
