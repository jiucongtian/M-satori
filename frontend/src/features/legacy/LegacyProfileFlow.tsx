...
export function MyHome({ name, balance, open, navigate }: { name: string; balance: number | null; open: (action: string) => void; navigate: (target: HomeNavTarget) => void }) {
  const items = [["账号与退出", "管理当前账号登录状态", "settings", "隐"], ["联系我们", "官方社交媒体与客服渠道", "support", "联"]] as const;
  return <section className="my-page my-home"><div className="my-home-scroll"><header><Brand compact /></header><button className="my-identity" type="button" onClick={() => open("profile")} aria-label="查看我的生命智慧档案"><div className="avatar-seed"><i /><span>{(name || "我").slice(0,1)}</span></div><div><p>你好</p><h1>{name || "我"}</h1><small>生命智慧档案已建立</small></div><b>›</b></button><button className="profile-banner" type="button" onClick={() => open("archive")} aria-label="进入生命智慧档案库"><div><small>MY LIFE WISDOM ARCHIVE</small><h2>生命智慧档案库</h2><p>查看和管理自己与重要之人的档案</p><strong className="profile-banner-action">进入档案库 <span>→</span></strong></div><div className="four-dots"><i /><i /><i /><i /></div><b>›</b></button><section className="my-r11-service-center"><header><div><small>MY SERVICES</small><h2>我的服务</h2></div><button type="button" onClick={() => open("orders")}>服务订单 ›</button></header><button className="my-current-membership" type="button" onClick={() => open("membership")}><div className="membership-mark"><span>和</span><i/></div><div><small>当前服务 · 体验版</small><strong>清和计划</strong><p>本周期还剩 18 天</p></div><em>正在使用</em><b>›</b></button><section className="my-quota-summary" aria-label="当前服务权益"><header><strong>当前可用权益</strong><button type="button" onClick={() => open("benefits")}>查看全部 ›</button></header><div><button type="button" onClick={() => open("benefits")}><span>日</span><p><small>今日能量</small><strong>12<em> / 15次</em></strong><b>2026.09.14刷新</b></p></button><button type="button" onClick={() => open("benefits")}><span>问</span><p><small>抽卡问事</small><strong>4<em> / 5份</em></strong><b>2026.09.14刷新</b></p></button></div></section><button className="my-seed-credit-secondary" type="button" onClick={() => open("seeds")}><span>芽</span><div><small>平台赠送 · AI体验额度</small><strong>智慧种子 {balance ?? "—"} 颗</strong><p>可体验指定AI功能，也可解锁指定活动价</p></div><b>明细 ›</b></button><p className="my-seed-boundary">不可购买、交易、提现或兑换人民币；活动价不等于现金抵扣。</p><div className="my-commerce-shortcuts r11"><button type="button" onClick={() => open("membership")}><i>会</i><span><strong>会员计划</strong><small>比较微光、清和、自在</small></span><b>›</b></button><button type="button" onClick={() => open("services")}><i>选</i><span><strong>服务商城</strong><small>购买明确服务与次数包</small></span><b>›</b></button></div></section><div className="my-menu">{items.map(([title,note,action,icon]) => <button type="button" key={title} onClick={() => open(action)}><i>{icon}</i><span><strong>{title}</strong><small>{note}</small></span><b>›</b></button>)}</div></div><HomeMainNav active="我的" navigate={navigate} /></section>;
}
