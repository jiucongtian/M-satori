...
const defaultShowPageDebugLabels = process.env.NEXT_PUBLIC_SHOW_PAGE_LABELS !== "false";
export function PageDebugLabel({ children }: { children: string }) { const [enabled,setEnabled]=useState(defaultShowPageDebugLabels); useEffect(()=>{let active=true; fetch("/api/v1/system-config/page-labels",{credentials:"include"}).then(r=>r.ok?r.json():null).then(payload=>{if(!active)return; const value=payload?.data?.enabled ?? payload?.enabled; if(typeof value === "boolean"){setEnabled(value); localStorage.setItem("wfh.showPageLabels",String(value));}}).catch(()=>{const cached=localStorage.getItem("wfh.showPageLabels"); if(cached!==null)setEnabled(cached!=="false")}); return()=>{active=false}},[]); return enabled ? <span className="screen-id" aria-hidden="true">{children}</span> : null; }
export function Brand({ compact = false }: { compact?: boolean }) { return <Link className={`brand ${compact ? "brand-compact" : ""}`} href="/" aria-label="五福荟首页"><img className="brand-lockup" src="/brand/wufuhui-lockup.webp" alt="五福荟 WU FU HUI" /></Link>; }
