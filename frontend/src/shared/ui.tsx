...
export function PageDebugLabel({ children }: { children: string }) { const [enabled,setEnabled]=useState(defaultShowPageDebugLabels); useEffect(()=>{const read=()=>{const storedValue=localStorage.getItem("wfh.showPageLabels");setEnabled(storedValue===null?defaultShowPageDebugLabels:storedValue==="true")}; read(); window.addEventListener("storage",read); return()=>window.removeEventListener("storage",read)},[]); return enabled ? <span className="screen-id" aria-hidden="true">{children}</span> : null; }
