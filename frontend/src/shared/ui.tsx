...
export function PageDebugLabel({ children }: { children: string }) { const [enabled,setEnabled]=useState(defaultShowPageDebugLabels); useEffect(()=>{const read=()=>setEnabled(localStorage.getItem("wfh.showPageLabels")!=="false"); read(); window.addEventListener("storage",read); return()=>window.removeEventListener("storage",read)},[]); return enabled ? <span className="screen-id" aria-hidden="true">{children}</span> : null; }
