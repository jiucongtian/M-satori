...
function Calculating({ name, onDone }: { name: string; onDone: () => void }) {
  return <div className="calculating"><div className="calc-orbit"><i /><i /><span>成档</span></div><p className="eyebrow">CREATING YOUR PROFILE</p><h1>正在为{name}<br />建立生命智慧档案</h1><div className="calc-stages"><span className="done">✓ 核对出生地点与时间</span><span className="done">✓ 对齐你的生命节律</span><span className="done">✓ 生成四张关系卡牌</span><span className="active">· 准备你的档案初见</span></div><p>你的档案初见已准备好</p><button className="text-action" type="button" onClick={onDone}>开启档案初见 →</button></div>;
}
