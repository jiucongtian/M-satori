...
function TodayHome({ name, home, onNext, navigate }: { name: string; home: HomeOverview | null; onNext: () => void; navigate: (step: number) => void }) {
  ...
  return <section className="today-home"><div className="life-growth" aria-hidden="true"><i className="living-seed" /><i className="living-stem" /><i className="living-leaf leaf-left" /><i className="living-leaf leaf-right" /><i className="living-leaf leaf-top" /><span className="growth-ring ring-one" /><span className="growth-ring ring-two" /></div><header><Brand compact /><div className="seed-balance" title="平台附赠的 AI 体验额度"><i>●</i><span>智慧种子</span><strong>{home?.wisdomSeedAccount.available ?? "—"}</strong></div></header><p className="eyebrow">TODAY · {home?.dailyInsight.localDate || new Date().toLocaleDateString("zh-CN")}</p><h1>{summary?.greeting || `${name}，你好`}</h1><p className="home-note">{summary ? `今日指引 · ${summary.date}` : "今日能量摘要暂未准备好，请稍后再试。"}</p><article className="daily-guide home-energy-card"><div className="energy-scale" aria-label={summary ? `今日能量为${summary.energyLevel}` : "今日能量暂不可用"}><small>今日能量</small><div>{(["高", "中", "低"] as const).map((level) => <span key={level} className={summary?.energyLevel === level ? "active" : ""}>{level}</span>)}</div></div><p className="guide-kicker">TODAY&apos;S GUIDANCE</p><h2>{summary?.guidance || "今日能量摘要暂不可用"}</h2>{summary ? <div className="guide-tips"><div><small>适合做什么</small><strong>{summary.suitableActions.map((item, index) => <span key={item}>{index > 0 && <br />}{item}</span>)}</strong></div><div><small>注意什么</small><strong>{summary.cautions.map((item, index) => <span key={item}>{index > 0 && <br />}{item}</span>)}</strong></div></div> : null}<button type="button" onClick={onNext}>{ready ? "查看今日能量指引" : "获取今日能量指引"} <span>→</span></button></article><MainNav active="今日" navigate={navigate} /></section>;
}
