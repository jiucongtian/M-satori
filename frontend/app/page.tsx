...
function TodayHome({ name, onNext, navigate }: { name: string; onNext: () => void; navigate: (step: number) => void }) {
  return <section className="today-home root-tab-page"><div className="life-growth" aria-hidden="true"><i className="living-seed" /><i className="living-stem" /><i className="living-leaf leaf-left" /><i className="living-leaf leaf-right" /><i className="living-leaf leaf-top" /><span className="growth-ring ring-one" /><span className="growth-ring ring-two" /></div><header><Brand compact /><div className="seed-balance"><i>●</i><span>智慧种子</span><strong>3</strong></div></header><p className="eyebrow">TODAY · 立秋</p><h1>{name}，早上好</h1><p className="home-note">今天不必急着证明什么，先让自己的节奏回来。</p><article className="daily-guide home-energy-card"><div className="energy-scale" aria-label="今日能量为中"><small>今日能量</small><div><span>高</span><span className="active">中</span><span>低</span></div></div><p className="guide-kicker">TODAY&apos;S GUIDANCE</p><h2>先稳住自己的节奏，再清醒回应外界的变化。</h2><div className="guide-tips"><div><small>适合做什么</small><strong>梳理重点<br />确认后再行动</strong></div><div><small>注意什么</small><strong>别急着回应<br />避免情绪消耗</strong></div></div><button type="button" onClick={onNext}>获取今日能量指引 <span>→</span></button></article><MainNav active="今日" navigate={navigate} /></section>;
}
...
function ComingSoonPage({ kind, navigate }: { kind: ComingSoonKind; navigate: (step: number) => void }) {
  ...
  return <section className="coming-soon-page root-tab-page">
...
function MyHome({ name, open }: { name: string; open: (step: number) => void }) {
  ...
  return <section className="my-page my-home root-tab-page"><header><Brand compact /><button className="my-message" type="button" onClick={() => open(28)}>信<i /></button></header><button className="my-identity" type="button" onClick={() => open(22)} aria-label="查看我的生命智慧档案"><div className="avatar-seed"><i /><span>{(name || "小满").slice(0,1)}</span></div><div><p>晚上好</p><h1>{name || "小满"}</h1><small>生命智慧档案已建立 · 共同成长第 1 天</small></div><b>›</b></button><button className="profile-banner" type="button" onClick={() => open(111)}><div><small>MY LIFE WISDOM ARCHIVE</small><h2>生命智慧档案库</h2><p>我的主档案 · 已保存 5 位重要的人</p></div><div className="four-dots"><i /><i /><i /><i /></div><b>→</b></button><div className="my-assets"><button type="button" onClick={() => open(23)}><span>●</span><div><small>智慧种子</small><strong>2</strong></div><b>›</b></button></div><div className="my-menu">{items.map(([title,note,step,icon]) => <button type="button" key={title} onClick={() => open(step)}><i>{icon}</i><span><strong>{title}</strong><small>{note}</small></span><b>›</b></button>)}</div><MainNav active="我的" navigate={open} /></section>;
}
...
function ArchiveConfirm({onBack,onNext}:{onBack:()=>void;onNext:()=>void}){const [accuracy,setAccuracy]=useState("准确");const notes:{[key:string]:string}={"准确":"已确认出生时间准确到分钟，将使用 06:30 生成四张卡牌。","大概时段":"系统会保留时间误差标记，报告中不会把不确定内容写成确定事实。","不知道":"仍可建立档案，但自我关系卡牌会标记为待补充。"};return <section className="my-page archive-page"><MyHeader title="确认出生信息" onBack={onBack}/><div className="confirm-person"><span>妈</span><p><small>即将创建</small><strong>妈妈的生命智慧档案</strong><b>家人 · 母亲</b></p></div><div className="archive-facts"><p><span>历法</span><strong>公历</strong></p><p><span>出生日期</span><strong>1964年3月12日</strong></p><p><span>出生时间</span><strong>{accuracy==="不知道"?"暂不清楚":accuracy==="大概时段"?"清晨 · 大概时段":"06:30 · 准确到分钟"}</strong></p><p><span>出生地点</span><strong>浙江省杭州市</strong></p></div><div className="accuracy-card"><strong>时间准确度</strong><div>{["准确","大概时段","不知道"].map(x=><button key={x} className={accuracy===x?"active":""} onClick={()=>setAccuracy(x)}>{x}</button>)}</div><p>{notes[accuracy]}</p></div><div className="privacy-inline"><span className="lock"/><p><strong>这是你的私人关系记录</strong><small>对方不会收到通知；共享、共同查看或互动前需另行授权。</small></p></div><button className="primary" onClick={onNext}>确认并生成四张卡牌 <span>→</span></button><button className="text-action" onClick={onBack}>返回修改</button></section>}
...
function ArchiveDeleteImpact({onBack,onDone}:{onBack:()=>void;onDone:()=>void}){const [confirmed,setConfirmed]=useState(false);return <section className="my-page archive-page delete-impact"><MyHeader title="删除档案" onBack={onBack}/><div className="delete-symbol">删</div><p className="eyebrow">PLEASE REVIEW THE IMPACT</p><h1>删除“妈妈”的档案前<br/>请确认这些影响</h1><div className="impact-list"><p><i>档</i><span><strong>四张关系卡牌将被删除</strong><small>出生资料与人物标签也会同时移除</small></span></p><p><i>∞</i><span><strong>2 份关系报告会保留为历史</strong><small>但不再支持重新生成或继续互动</small></span></p><p><i>迹</i><span><strong>时间线引用会显示“源档案已删除”</strong><small>你仍可单独删除这些历史记录</small></span></p></div><label className="delete-confirm"><input type="checkbox" checked={confirmed} onChange={e=>setConfirmed(e.target.checked)}/><span>我了解删除无法撤销，并确认删除这份人物档案</span></label><button className="danger-primary" disabled={!confirmed} onClick={onDone}>确认永久删除</button><button className="outline-button" onClick={onBack}>暂不删除</button></section>}
