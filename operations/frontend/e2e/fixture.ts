...
export async function installApiFixture(page:Page){
 ...
 await page.route('**/operations-api/**',async route=>{
  ...
  const data=path.startsWith('/users')?(query&&!query.includes('131')?[]:users):path==='/products'?products:path==='/orders'?orders:path==='/dashboard'?{new_users:1,pending_orders:0,open_cases:1,critical_cases:1}:path==='/analytics/overview'?analytics:path==='/analytics/segments'?analyticsSegments:path==='/me'?{phone:'18108291023',nickname:'测试管理员',roles:[{code:'SUPER_ADMIN',name:'超级管理员'}]}:path==='/meta/operation-roles'?[{code:'SUPER_ADMIN',name:'超级管理员'}]:path==='/accounts'?[]:path==='/benefits'?[]:path==='/audit'?[]:path==='/action-requests'?[]:[];
