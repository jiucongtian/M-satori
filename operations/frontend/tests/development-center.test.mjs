...
test('总览分别呈现四端真实提交口径且账户区禁止扭曲换行',async()=>{const [center,css]=await Promise.all([readFile(new URL('../app/development-center.tsx',import.meta.url),'utf8'),readFile(new URL('../app/development-center-spec.css',import.meta.url),'utf8')]);assert.match(center,/commitsCapped\?'100\+'/);assert.match(center,/测试文件占比/);assert.match(center,/前 300 个文件小计/);assert.match(css,/admin-account strong/);assert.match(css,/text-overflow:ellipsis/);assert.match(css,/white-space:nowrap/)});
