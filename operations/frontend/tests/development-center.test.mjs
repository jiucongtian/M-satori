...
test('总览呈现精确提交数且账户区禁止扭曲换行',async()=>{const [center,css,views]=await Promise.all([readFile(new URL('../app/development-center.tsx',import.meta.url),'utf8'),readFile(new URL('../app/development-center-spec.css',import.meta.url),'utf8'),readFile(new URL('../app/development-views.tsx',import.meta.url),'utf8')]);assert.doesNotMatch(center,/100\+/);assert.match(center,/show\(x.commits\)/);assert.match(center,/测试文件占比/);assert.match(views,/releaseTotals/);assert.match(css,/admin-account strong/);assert.match(css,/white-space:nowrap/)});
