...
async function fetchDevelopmentCenterData(release:ReleaseName){
 ...
 const componentCommits=await Promise.all(paths.map(path=>github<Commit[]>(`/commits?sha=${encodeURIComponent(branch)}&path=${encodeURIComponent(path)}&per_page=100`).catch(()=>[])));
 const differenceTruncated=files.length>=300;
 const components=names.map((name,index)=>{const path=paths[index],ownCommits=componentCommits[index],changed=files.filter(x=>componentOf(x.filename)===name),sources=tree.filter(x=>x.type==='blob'&&x.path.startsWith(`${path}/`)&&/\.(ts|tsx|js|mjs)$/.test(x.path)),tests=sources.filter(x=>/(test|spec)\.(ts|tsx|js|mjs)$/.test(x.path));return {name,path,sha:ownCommits[0]?.sha.slice(0,7)||'不可得',additions:files.length&&!differenceTruncated?changed.reduce((n,x)=>n+x.additions,0):null,deletions:files.length&&!differenceTruncated?changed.reduce((n,x)=>n+x.deletions,0):null,differenceComplete:files.length>0&&!differenceTruncated,differenceReason:differenceTruncated?'GitHub 差异超过 300 个文件上限':files.length?null:'GitHub 差异数据未返回',commits:ownCommits.length||null,commitsCapped:ownCommits.length>=100,testFileRate:sources.length?Math.round(tests.length/sources.length*100):null,testCases:tests.length,sourceFiles:sources.length,deployment:name.startsWith('运营')?'测试环境运行中':'待部署回执'}});
 ...
 const data={source:{mode:failures.length||differenceTruncated?'PARTIAL':'LIVE',updatedAt:new Date().toISOString(),message:failures.length?`以下 GitHub 数据源读取失败：${failures.join('、')}；其余指标来自本次快照。`:differenceTruncated?'GitHub 对比接口仅返回前 300 个变更文件，代码增删为可见文件小计，不代表 Release 全量。':'GitHub 数据来自每日快照；部署状态来自测试环境配置。',repository},releases:['R1.1','R1.0'],release:{name:release,stage:release==='R1.1'?'测试中':'历史版本',environment:'测试环境',tag:release==='R1.1'?'尚未封版':'历史标签',branch,startedAt:first?new Date(first.commit.author.date).toLocaleString('zh-CN'):'不可得',taggedAt:null,duration:first?`${Math.max(1,Math.ceil((Date.now()-new Date(first.commit.author.date).getTime())/86400000))} 天`:'不可计算'},components,
