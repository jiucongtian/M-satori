...
async function fetchDevelopmentCenterData(release:ReleaseName){
 ...
 const components=names.map((name,index)=>{const path=paths[index],ownCommits=componentCommits[index],changed=files.filter(x=>componentOf(x.filename)===name),sources=tree.filter(x=>x.type==='blob'&&x.path.startsWith(`${path}/`)&&/\.(ts|tsx|js|mjs)$/.test(x.path)),tests=sources.filter(x=>/(test|spec)\.(ts|tsx|js|mjs)$/.test(x.path));return {name,path,sha:ownCommits[0]?.sha.slice(0,7)||'不可得',additions:files.length?changed.reduce((n,x)=>n+x.additions,0):null,deletions:files.length?changed.reduce((n,x)=>n+x.deletions,0):null,differenceComplete:files.length>0&&!differenceTruncated,commits:ownCommits.length||null,commitsCapped:ownCommits.length>=100,testFileRate:sources.length?Math.round(tests.length/sources.length*100):null,testCases:tests.length,sourceFiles:sources.length,deployment:name.startsWith('运营')?'测试环境运行中':'待部署回执'}});
