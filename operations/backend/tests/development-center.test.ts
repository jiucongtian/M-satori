...
test('研发数据按北京时间每日快照读取并为不可得指标保留 null',async()=>{const source=await readFile(new URL('../src/development-center.ts',import.meta.url),'utf8');assert.match(source,/api\.github\.com/);assert.match(source,/operations_development_snapshots/);assert.match(source,/setUTCHours\(15,0,0,0\)/);assert.match(source,/captured_by/);assert.match(source,/functionCoverage:null/);assert.match(source,/codeCoverage:null/);assert.match(source,/reopened:null/)});
...
test('完整 Git 历史使用独立凭据且公共仓库可匿名采集',async()=>{const source=await readFile(new URL('../src/development-git.ts',import.meta.url),'utf8');assert.match(source,/OPERATIONS_GIT_TOKEN/);assert.doesNotMatch(source,/OPERATIONS_GITHUB_TOKEN/);assert.match(source,/GIT_TERMINAL_PROMPT:'0'/)});
