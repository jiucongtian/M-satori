...
当前版本可使用环境变量 `OPERATIONS_ACCOUNTS_JSON` 配置一个或多个系统引导管理员；旧版单账号变量仍兼容一个发布周期。引导管理员均拥有超级管理员权限，密码只保存 SHA-256 十六进制摘要。登录后还可以在平台内创建手机号运营账号并分配角色，这类账号使用带随机盐的 scrypt 密码摘要。生产环境必须为每个账号使用独立强密码及至少 32 字节 JWT 密钥，不得将用户端 Token 当作运营权限。
