...
- `subjectType` 支持 `SELF`、`OTHER`。不能把当前选中的旧牌库视为“本人”；一个目标用户只能选一份本人档案，已有本人档案时整批拒绝覆盖。
- `relationshipType` 支持 `FAMILY`、`FRIEND`、`COLLEAGUE`、`OTHER`。旧资料没有记录关系时可用 `OTHER`。
...
选中批次整体原子提交。任一档案失败时整批回滚；已存在的本人档案、跨账号认领、软删除的既往导入或计划变化会阻断。长期防重使用确定性的 `audit_logs` 标记，不依赖会过期的幂等缓存。同一批次重跑返回 `REPLAYED`，不会新增档案。
...
目前已交付离线迁移脚本与测试。正式导入前仍需真实账号核验、出生地与时间精度确认，并处理隔离档案；未部署认领页面，也未迁移任何线上用户。
