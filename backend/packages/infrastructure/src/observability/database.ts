  ...
  return function (this: unknown, ...args: unknown[]) {
    ...
    const finish = (error?: unknown) => {
      ...
      if (error || durationMs >= (metric === 'db_acquire_ms' ? 100 : 250)) {
        logEvent(error ? `${metric}_failed` : `${metric}_slow`, { ...context, ...fields, durationMs, ...errorFields(error) }, error ? 'warn' : 'info');
      }
    };
    const last = args.at(-1);
    if (typeof last === 'function') {
      args[args.length - 1] = (...result: unknown[]) => { finish(result[0]); return correlation.run(context ?? {}, () => Reflect.apply(last, undefined, result)); };
    }
    try {
      const result = Reflect.apply(original, this, args);
      if (result instanceof Promise) return result.then((value: unknown) => { finish(); return value; }, (error: unknown) => { finish(error); throw error; });
      return result;
    } catch (error) { finish(error); throw error; }
  };
}
export function instrumentDatabase(pool: Pool): void {
  pool.connect = instrumentCall(pool.connect as Callable, 'db_acquire_ms', {}) as Pool['connect'];
  const seen = new WeakSet<PoolClient>();
  pool.on('connect', (client: PoolClient) => {
    if (seen.has(client)) return;
    seen.add(client);
    const original = client.query as Callable;
