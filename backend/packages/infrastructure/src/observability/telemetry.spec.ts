...
import { MetricWindow, correlation, correlationIds, errorFields, logEvent, trackActive, activeGauges } from './telemetry.js';

describe('telemetry', () => {
  afterEach(() => vi.restoreAllMocks());
  it('keeps full interval histogram counts with bounded cardinality and resets windows', () => {
    const window = new MetricWindow();
    for (const value of [1, 1, 100, 1000, 1_000_000]) window.observe('latency', value, { route: '/tasks/:id' });
    ...
    expect(bounded.series).toHaveLength(2048); expect(bounded.overflow).toBe(2);
  });
  it('logs a single JSON record and excludes error bodies and non-identifier fields', () => {
    const spy = vi.spyOn(console, 'info').mockImplementation(() => {});
    correlation.run({ requestId: 'request-1' }, () => logEvent('failure', { ...correlationIds({ taskId: 'task-1', token: 'secret' }), ...errorFields(Object.assign(new Error('secret'), { code: 'ETIMEDOUT', response: 'secret' })) }));
    ...
    spy.mockImplementation(() => { throw new Error('sink unavailable'); });
    expect(() => logEvent('safe')).not.toThrow();
  });
  it('releases active counters once even on multiple close paths', () => {
    const close = trackActive('testActive'); expect(activeGauges().testActive).toBe(1);
    close(); close(); expect(activeGauges().testActive).toBe(0);
  });
});
