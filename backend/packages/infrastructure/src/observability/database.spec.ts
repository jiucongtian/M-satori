...
describe('database telemetry compatibility', () => {
  ...
  it('keeps callbacks, promises, SQL parameters and errors intact while restoring checkout context', async () => {
    ...
    const client = { query: vi.fn((...args: unknown[]) => {
      const callback = args.at(-1);
      const error = args[0] === 'bad' ? queryError : undefined;
      if (typeof callback === 'function') { queueMicrotask(() => callback(error, { rows: [1] })); return; }
      return error ? Promise.reject(error) : Promise.resolve({ rows: [1] });
    }) } as unknown as PoolClient;
    const original = client.query;
    pool.connect = vi.fn((callback?: typeof connectCallback) => { if (callback) { connectCallback = callback; return; } return Promise.resolve(client); }) as Pool['connect'];
    instrumentDatabase(pool); pool.emit('connect', client); pool.emit('connect', client);
    const logs = vi.spyOn(console, 'info').mockImplementation(() => {});
    let context: unknown;
    correlation.run({ requestId: 'request-B' }, () => {
      pool.connect((_error, _client, done) => { context = correlation.getStore(); done(); });
    });
    correlation.run({ requestId: 'request-A' }, () => connectCallback!(undefined, client, release));
    expect(context).toEqual({ requestId: 'request-B' }); expect(release).toHaveBeenCalledOnce();
    ...
    await new Promise<void>((resolve) => client.query('bad', (error) => { expect(error).toBe(queryError); resolve(); }));
    await expect(client.query('bad')).rejects.toBe(queryError);
    const rows = metrics.drain().series;
    expect(rows.filter((row) => row.metric === 'db_acquire_ms').reduce((sum, row) => sum + row.count, 0)).toBe(2);
    expect(rows.filter((row) => row.metric === 'db_query_ms').reduce((sum, row) => sum + row.count, 0)).toBe(3);
