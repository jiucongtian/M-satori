...
class TestController {
  @Get('money-orders/:id') async ok() {
    await new Promise((resolve) => setTimeout(resolve, 2));
    return { orderId: 'order-1', context: correlation.getStore() };
  }
  @Get('denied') denied() {
    throw new UnauthorizedException('secret-token');
  }
  @Get('broken') broken() {
...
describe('request telemetry', () => {
  ...
  it('records final error status once, hides query/body, isolates overlapping request contexts', async () => {
    ...
    const app = await NestFactory.create<NestFastifyApplication>(TestModule, createFastifyAdapter(), {
      logger: false,
    });
    ...
    try {
      ...
      const results = await Promise.all(
        ids.map((id) =>
          app.inject({
            method: 'GET',
            url: '/api/v1/money-orders/private-id?token=secret-token',
            headers: { 'x-request-id': id },
          }),
        ),
      );
      results.forEach((result, i) =>
        expect(result.json<{ context: { requestId: string } }>().context.requestId).toBe(ids[i]),
      );
      ...
      const records = logs.mock.calls
        ...
        .filter((row) => row.event === 'http_request_completed');
