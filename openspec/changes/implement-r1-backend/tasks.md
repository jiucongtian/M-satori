...
- [ ] 9.1 按冻结契约实现 `/api/v1/me/life-profiles*` 游标列表、SELF/OTHER 创建和一个 SELF 约束
- [ ] 9.2 为他人档案复用 preview/confirm revision、四张卡牌和版本历史能力
- [ ] 9.3 实现档案元数据更新、owner/subject 查询隔离和跨用户统一 404
- [ ] 9.4 实现档案删除影响检查、活动任务阻塞、逻辑删除和异步清理
- [ ] 9.5 按冻结契约实现 `/api/v1/me/life-profile-groups*` 创建、重命名、排序和删除
- [ ] 9.6 实现档案加入/移出分组及删除分组时原子清空关联但保留档案
- [ ] 9.7 添加第二 SELF、跨用户枚举、他人档案 revision、分组事务、删除影响和历史引用测试
