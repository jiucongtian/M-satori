...
- [x] 9.10 按范围决策使用服务端 Fake 自动成功完成端到端联调，真实微信测试商户联调后置为独立变更
- [x] 9.11 完成 Mock 支付事实、金额一致性、资金/权益对账和安全评审后提交推送独立里程碑
...
- [x] 11.14 完成本阶段会员与资金/权益对账后提交推送独立里程碑
...
- [x] 13.1 对接商品商城、商品详情、会员方案、报价、订单确认、按 Provider 区分的支付发起和支付结果页面
...
- [x] 14.6 在测试环境部署前先拉取服务器最新代码，处理并确认合并结果后执行 expand migration
- [x] 14.7 在测试环境迁移种子 opening grants，核对新旧 available/reserved 与流水报告并保持旧读模型回退开关
- [x] 14.8 使用 Feature Flag 按商品报价 → 已有权益核销 → 权益包支付 → 会员开通续费 → 普通退款 → 会员升级顺序灰度
- [x] 14.9 验证关闭新订单后仍能完成或补偿所有已支付订单，并演练 API/Worker/支付回调/数据库兼容回滚
- [x] 14.10 完成 Mock 测试里程碑技术 Go/No-Go 与交付证据归档；真实支付财务和商户验收后置到独立变更
- [x] 14.11 更新 R1.1 交接文档、接口状态、部署说明、对账说明和已知限制
- [x] 14.12 完成最终回归、OpenSpec 状态核验并提交推送发布里程碑
