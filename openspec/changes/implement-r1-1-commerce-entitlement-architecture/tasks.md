...
- [ ] 2.8 完成本阶段 typecheck、lint、模块装配测试并提交推送独立里程碑
...
- [ ] 4.1 实现 ServiceOffering 与不可变 OfferingVersion repository、发布命令和历史查询
- [ ] 4.2 实现 R1.1 可售范围白名单，拒绝生命之光、月运、年运、关系报告、真人服务和实体商品付款
- [ ] 4.3 实现今日能量单次/新客包、问事单次/10次包及微光/清和/自在三档 30 天方案种子数据
- [ ] 4.4 实现 SeedPromotionRule 的业务空间、身份、商品、种子门槛、活动人民币价、限次、有效期和恢复规则
- [ ] 4.5 实现服务端 CheckoutQuote 计算、用户绑定、商品/活动资格/限购校验和 15 分钟过期
- [ ] 4.6 实现标准价与种子活动价返回，确保不输出固定种子汇率、人民币抵扣或组合支付表达
- [ ] 4.7 实现商品列表、详情、会员方案和创建报价接口及 OpenAPI 契约测试
- [ ] 4.8 添加并发新客限购、商品下架、版本变更、报价过期和活动资格边界测试
- [ ] 4.9 完成本阶段单元/契约/集成测试并提交推送独立里程碑
...
- [ ] 9.1 实现从有效 CheckoutQuote 创建 MoneyOrder、保存完整快照、限购原子校验和幂等重放
- [ ] 9.2 实现未支付订单 30 分钟自动关闭及种子活动 reservation 的准确释放
- [ ] 9.3 定义 PaymentProvider 并实现确定性 Fake、故障注入和 WeChat Pay 适配器配置
- [ ] 9.4 实现创建 PaymentAttempt、客户端支付参数、主动查单和支付状态查询
- [ ] 9.5 实现微信支付回调验签、商户/订单/金额/币种复核、原文最小化保存和 provider event 去重
- [ ] 9.6 实现一个订单最多一个成功资金事实和重复/乱序回调安全处理
- [ ] 9.7 在支付成功事务内更新支付事实、消耗活动种子 reservation 并写入 FulfillmentRequested Outbox
- [ ] 9.8 实现订单列表/详情、支付尝试和支付结果接口，区分 paid 与 fulfilled
- [ ] 9.9 添加报价过期、重复下单、支付取消/失败/处理中、回调伪造/重复/乱序和关闭竞争测试
- [ ] 9.10 使用微信沙箱或测试商户完成签名、回调、主动查单和金额一致性联调
- [ ] 9.11 完成本阶段资金对账和安全评审后提交推送独立里程碑

## 10. 交付 Saga 与异常补偿

- [ ] 10.1 实现 FulfillmentJob 状态机、Outbox 消费、重试、退避、超时、死信和幂等业务键
- [ ] 10.2 实现按订单快照发放单次权益包、10次权益包或创建会员周期的交付命令
- [ ] 10.3 实现支付成功但交付未完成时 `PAID/FULFILLING` 查询和前端提示
- [ ] 10.4 实现交付成功后订单 FULFILLED、来源权益引用和事件审计
- [ ] 10.5 实现交付最终失败的异常原路退回请求并防止用户重复购买
- [ ] 10.6 实现支付—订单—交付—权益/会员关联对账和可重放修复任务
- [ ] 10.7 添加 Outbox 重投、Worker 崩溃、权益发放重复、会员创建重复和最终失败补偿测试
- [ ] 10.8 完成“权益不足 → 下单 → 支付 → 发放 → 返回原业务上下文”的端到端测试

## 11. 会员周期、续费与替换式升级

- [ ] 11.1 实现 MembershipPlanVersion 到 MembershipSubscription/Period 的快照映射
- [ ] 11.2 实现同一业务空间单一 ACTIVE 周期和有序未来续费周期约束
- [ ] 11.3 实现无有效会员时支付交付后立即开始 30 天周期并幂等发放服务权益
- [ ] 11.4 实现有效会员手动续费排队、周期结束后自动开始且不提前发放下一周期权益
- [ ] 11.5 实现周期结束、未使用会员权益到期且不结转的只追加流水
- [ ] 11.6 实现允许的微光→清和、微光→自在、清和→自在升级校验和全额新订单
- [ ] 11.7 实现 MembershipUpgrade Saga：新方案安全激活后终止旧周期并作废旧权益
- [ ] 11.8 实现 remainingTimeRatio、remainingQuotaRatio、residualValueEstimate 和 assessmentRuleVersion 内部快照
- [ ] 11.9 保证剩余价值不改变新订单金额、不创建 Refund、不形成财务应付且不进入用户退款页面
- [ ] 11.10 实现升级确认文案契约，明确原方案结束和剩余次数不保留但不展示退款规则
- [ ] 11.11 实现新计划无法履约时保留旧会员并处理新订单异常退回的补偿路径
- [ ] 11.12 实现会员详情、周期、续费和升级状态 API 及后台查询
- [ ] 11.13 添加周期边界、重复 Worker、并发续费/升级、权益不结转和升级中断恢复测试
- [ ] 11.14 完成本阶段会员与资金/权益对账后提交推送独立里程碑

## 12. 普通退款、后台调整与对账运营

- [ ] 12.1 实现版本化 RefundPolicy、退款报价、资格校验和订单快照规则读取
- [ ] 12.2 实现普通退款申请时冻结剩余权益、支付渠道原路退款、成功反向流水和失败重试
- [ ] 12.3 实现重复扣款及最终无法履约的系统自动异常原路退回
- [ ] 12.4 明确阻断会员升级旧周期剩余价值进入任何退款接口或 PaymentProvider 退款命令
- [ ] 12.5 实现受 RBAC 保护的补发、释放、恢复、作废和冲正命令，强制原因码、操作者与关联业务 ID
- [ ] 12.6 实现订单、支付、退款、交付、权益、种子、消费意图和会员的运营查询视图
- [ ] 12.7 实现日常自动对账、异常 case 生命周期、指标、告警和可审计修复流程
- [ ] 12.8 添加越权、直接改余额防护、重复退款、退款/核销竞争和人工调整审计测试

## 13. H5 联调、可观测性与安全

- [ ] 13.1 对接商品商城、商品详情、会员方案、报价、订单确认、微信支付和支付结果页面
- [ ] 13.2 对接我的权益、来源分组、使用记录、订单、普通退款、会员周期和升级页面
- [ ] 13.3 对接固定核销提示，确保前端不提供切换来源入口且不自行计算价格或优先级
- [ ] 13.4 保存支付返回前的 opaque businessContext 并验证支付后恢复原问事流程
- [ ] 13.5 增加 requestId、orderId、paymentAttemptId、fulfillmentId、subscriptionId、grantId 和 consumptionIntentId 关联日志
- [ ] 13.6 增加报价转化、支付结果、交付延迟、悬挂预留、账本不一致、周期漏发和对账异常指标
- [ ] 13.7 完成支付证书/密钥管理、回调网络限制、敏感字段脱敏、商业模块不读取问事正文和审计安全检查
- [ ] 13.8 编写支付故障、悬挂预留、交付失败、账本修复、会员周期与对账 Runbook

## 14. 质量门禁、迁移与发布

- [ ] 14.1 执行全部六份 capability 的场景追踪和 OpenSpec/OpenAPI 严格校验
- [ ] 14.2 完成商品报价、服务权益、种子账本、固定核销、订单支付交付和会员生命周期单元测试门禁
- [ ] 14.3 完成 PostgreSQL 真实事务、行锁、迁移、Outbox/Inbox 和对账集成测试
- [ ] 14.4 完成重复点击、多端并发、弱网重试、回调乱序、Worker 崩溃、跨日到期和最终补偿 E2E
- [ ] 14.5 执行商品/报价/订单/权益查询 P95、并发预留、支付回调和 Worker 积压压测并冻结 SLO
...
- [ ] 14.11 更新 R1.1 交接文档、接口状态、部署说明、对账说明和已知限制
